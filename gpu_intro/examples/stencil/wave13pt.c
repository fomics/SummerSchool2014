...
#else
#define _A(array, is, iy, ix) (array##p[is][iy][ix])
#endif
...
{
#if !defined(__PGI)
	real (*w0p)[ny][nx] = (real(*)[ny][nx])w0;
	real (*w1p)[ny][nx] = (real(*)[ny][nx])w1;
	real (*w2p)[ny][nx] = (real(*)[ny][nx])w2;
#endif
#if defined(_MIC)
	__assume_aligned(w0, MEMALIGN);
	__assume_aligned(w1, MEMALIGN);
	__assume_aligned(w2, MEMALIGN);
#endif
#if defined(_OPENACC)
