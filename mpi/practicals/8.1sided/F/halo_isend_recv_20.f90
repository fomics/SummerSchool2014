...
! INTEGER, PARAMETER :: number_package_sizes=9
  ...
  INTEGER :: rq(2) 
  INTEGER :: status(MPI_STATUS_SIZE), status_arr(MPI_STATUS_SIZE,2)
  REAL, ASYNCHRONOUS :: snd_buf_left(max_length), snd_buf_right(max_length)
  REAL :: rcv_buf_left(max_length), rcv_buf_right(max_length)
...
!              halo-communication                 halo-communication
  ...
  DO j = 1, number_package_sizes
     ...
     DO i = 0, number_of_messages
        ...
        CALL MPI_ISEND(snd_buf_left,  length, MPI_REAL, left,  17, MPI_COMM_WORLD, rq(1), ierror)
        CALL MPI_ISEND(snd_buf_right, length, MPI_REAL, right, 23, MPI_COMM_WORLD, rq(2), ierror)

        CALL MPI_RECV (rcv_buf_right, length, MPI_REAL, right, 17, MPI_COMM_WORLD, status, ierror)
        CALL MPI_RECV (rcv_buf_left,  length, MPI_REAL, left,  23, MPI_COMM_WORLD, status, ierror)

        CALL MPI_WAITALL(2, rq, status_arr, ierror)
        CALL MPI_GET_ADDRESS(snd_buf_left,  iadummy, ierror)
        CALL MPI_GET_ADDRESS(snd_buf_right, iadummy, ierror)
!       ... or with MPI-3.0 and later:
!       IF (.NOT.MPI_ASYNC_PROTECTS_NONBLOCKING) CALL MPI_F_sync_reg(snd_buf_left)
!       IF (.NOT.MPI_ASYNC_PROTECTS_NONBLOCKING) CALL MPI_F_sync_reg(snd_buf_right)
