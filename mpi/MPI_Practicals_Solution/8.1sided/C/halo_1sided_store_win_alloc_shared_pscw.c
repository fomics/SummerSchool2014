...
/*                                rcv_buf_right(xxx) in process 'right':                                       */
  ...
  { 
    ...
    for (i = 0; i <= number_of_messages; i++)
    {
      ...
      MPI_Win_post (grp_left,  MPI_MODE_NOSTORE, win_rcv_buf_left);
      MPI_Win_post (grp_right, MPI_MODE_NOSTORE, win_rcv_buf_right);
      MPI_Win_start(grp_right, 0, win_rcv_buf_left);
      MPI_Win_start(grp_left,  0, win_rcv_buf_right);
