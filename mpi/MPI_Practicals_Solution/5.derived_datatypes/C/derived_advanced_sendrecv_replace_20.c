...
/* if (left == -1) left = size-1;*/
  ...
  MPI_Get_address(&buf.i, &first_var_address);
  MPI_Get_address(&buf.f, &second_var_address);
  ...
  /* Compute global sum. */
  sum.i = 0;        sum.f = 0;
  buf.i = my_rank;  buf.f = my_rank;  /* Step 1 = init */

  for( i = 0; i < size; i++) 
  {
    MPI_Sendrecv_replace (&buf, 1, datatype, right, to_right,  /* Step 2 */
                                             left, to_right,   /* Step 3 */
                          MPI_COMM_WORLD, &status);
    /* Step 4 is substituted by ..._replace */
