...
! Get nearest neighbour ranks.
  ...
  CALL MPI_CART_SHIFT(slice_comm, 0, 1, left, right, ierror)
...
! Compute sum.
  ...
  DO i = 1, size_of_slice
     ...
     CALL MPI_ISSEND(snd_buf, 1, MPI_INTEGER, right, to_right, slice_comm, request, ierror)

     CALL MPI_RECV(rcv_buf, 1, MPI_INTEGER, left, to_right, slice_comm, status, ierror)
